...
const OWNERSHIP_RULES: Record<string, (user: User, resourceId: string, resource: any) => boolean> = {
  appointment: (user: User, resourceId: string, resource: any) => {
    if (user.role === 'patient') return resource.patientId === user.id;
    if (user.role === 'hcw') return resource.hcwId === user.id;
    if (['receptionist', 'admin'].includes(user.role)) return true;
    return false;
  },
  medical_record: (user: User, resourceId: string, resource: any) => {
    if (user.role === 'patient') return resource.patientId === user.id;
    if (['hcw', 'nurse', 'pharmacist', 'lab_technician'].includes(user.role)) return true;
    return false;
  },
  bill: (user: User, resourceId: string, resource: any) => {
    if (user.role === 'patient') return resource.patientId === user.id;
    if (['accountant', 'admin'].includes(user.role)) return true;
    return false;
  },
  prescription: (user: User, resourceId: string, resource: any) => {
    if (user.role === 'patient') return resource.patientId === user.id;
    if (['hcw', 'pharmacist', 'nurse'].includes(user.role)) return true;
    return false;
  },
};
 ...
 */
export const checkOwnership = (resourceType: string, getResource: (req: Request) => any) => {
...
export const requireSameOrganization = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (requestOrgId && requestOrgId !== user.currentOrganization?.id) {
 ...
 */
export const canUserPerform = (user: User | null, action: string): boolean => {
  if (!user) return false;
