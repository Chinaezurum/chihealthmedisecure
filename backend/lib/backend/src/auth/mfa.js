 ...
 */
router.post('/setup/totp', body('userId').not().isEmpty(), async (req, res) => {
    ...
    try {
        ...
        const otpauthUrl = authenticator.keyuri(user.email, 'ChiHealth MediSecure', secret);
 ...
 */
router.post('/verify/totp-setup', body('userId').not().isEmpty(), body('code').isLength({ min: 6, max: 6 }), body('secret').not().isEmpty(), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ message: 'Invalid input' });
    }
 ...
 */
router.post('/verify/totp', body('userId').not().isEmpty(), body('code').isLength({ min: 6, max: 6 }), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ message: 'Invalid input' });
    }
 ...
 */
router.post('/verify/backup-code', body('userId').not().isEmpty(), body('code').not().isEmpty(), async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ message: 'Invalid input' });
    }
 ...
 */
router.post('/setup/webauthn/start', body('userId').not().isEmpty(), body('userName').not().isEmpty(), body('userEmail').isEmail(), async (req, res) => {
    try {
        ...
        const optionsJSON = Object.assign(Object.assign({}, options), { challenge: challenge.toString('base64'), user: Object.assign(Object.assign({}, options.user), { id: Buffer.from(userId).toString('base64') }) });
 ...
 */
router.post('/setup/webauthn/complete', body('userId').not().isEmpty(), body('credential').isObject(), async (req, res) => {
    try {
        ...
        const user = await db.getUserById(userId);
        const existingMethod = user === null || user === void 0 ? void 0 : user.mfaMethod;
        ...
        await db.updateUserMfa(userId, {
            ...
            backupCodes: hashedCodes,
            mfaEnrolledAt: (user === null || user === void 0 ? void 0 : user.mfaEnrolledAt) || new Date().toISOString()
        });
        return res.json({
            success: true,
 ...
 */
router.post('/verify/webauthn/start', body('userId').not().isEmpty(), async (req, res) => {
    try {
        const { userId } = req.body;
        const user = await db.getUserById(userId);
        ...
        const optionsJSON = Object.assign(Object.assign({}, options), { challenge: challenge.toString('base64'), allowCredentials: user.webAuthnCredentials.map(cred => ({
                type: 'public-key',
                id: cred.id
            })) });
        return res.json({ options: optionsJSON });
 ...
 */
router.post('/verify/webauthn/complete', body('userId').not().isEmpty(), body('credential').isObject(), async (req, res) => {
    try {
        ...
        const user = await db.getUserById(userId);
        if (user === null || user === void 0 ? void 0 : user.webAuthnCredentials) {
            const updatedCredentials = user.webAuthnCredentials.map(cred => cred.id === credential.id
                ? Object.assign(Object.assign({}, cred), { lastUsed: new Date().toISOString() }) : cred);
 ...
 */
router.delete('/credentials/:credentialId', body('userId').not().isEmpty(), async (req, res) => {
    try {
        ...
        const updatedCredentials = user.webAuthnCredentials.filter(cred => cred.id !== credentialId);
        await db.updateUserMfa(userId, { webAuthnCredentials: updatedCredentials });
 ...
 */
router.post('/backup-codes/regenerate', body('userId').not().isEmpty(), async (req, res) => {
    try {
        const { userId } = req.body;
        const backupCodes = generateBackupCodes();
        const hashedCodes = backupCodes.map(code => hashBackupCode(code));
 ...
 */
router.post('/disable', body('userId').not().isEmpty(), body('password').not().isEmpty(), async (req, res) => {
    try {
